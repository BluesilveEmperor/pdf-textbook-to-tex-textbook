...
**编码规范**：项目源码中公式定界符只允许两种形式——

- 行内公式：`$...$`（**禁用** `\(...\)`）
- 行间公式：`$$...$$`（**禁用** `\[...\]`）

**为什么需要后处理**：pandoc 把 Markdown 数学转成 LaTeX 时会做定界符改写——`$…$` → `\(...\)`、`$$…$$` → `\[...\]`（pandoc 无选项可关闭此行为）。因此逐章转换后必须把定界符**回替换**为美元符号形式。

**工具**：`scripts/fix_math_delimiters.py`

```powershell
python scripts/fix_math_delimiters.py <LaTeX项目根目录>
```
...
⚠️ **必须用负向后行断言 `(?<!\\)\\\[`**：`\\[2pt]`（`\\` 换行命令 + 行距可选参数）中第 2 个反斜杠后紧跟 `[`，朴素正则会把它的 `\[` 误当定界符，替换后代码直接损坏。这种写法在 `align` / `array` 内很常见，是此替换**唯一的隐蔽坑**。`\(` 同理加断言（排除 `\\(` 误匹配）。

⚠️ **`\left\[` / `\right\]` 预检查**：转义方括号定界符中的字面 `\[` 前是普通字母，断言排除不了。pandoc 不会产出这种写法，但 OCR 源可能出现；脚本检测到时会打印 `[WARN]`，须先手工改写为 `\left[` / `\right]` 再重跑。

**边界**：`equation` / `align` / `gather` 等 amsmath 环境**不是"符号包裹"形式**，不受本规范约束（带编号公式、多行对齐仍用环境）。脚本不触碰任何 `\begin{...}` 环境。

**时机**：在 OCR 公式修复（§7）与全角替换（§7.5）之后、编译验证之前执行；脚本幂等，可重复运行。替换后须重编译验证（新增章节后同理）。
